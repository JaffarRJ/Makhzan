...
export default {
    ...
    methods: {
        setList() {
            this.axios.get(`api/admin/transaction/listing`).then((res) => {
                this.getlist = res.data.data.data;
            });
        },
        addList() {
            this.axios.post('api/admin/transaction/store', this.list).then(response => (this.setUsers()))
                .catch(err => console.log(err))
                .finally(() => this.loading = false)
        },
        deleteList(id) {
            if (confirm("Are you sure to delete ?")) {
                this.axios.post(`api/admin/transaction/delete/`, {'id': id}).then(response => {
                    window.location.reload();
                });
            }
        }, editList(id) {
            this.axios.post(`api/admin/transaction/detail/`, {'id': id}).then(response => {
            ...
            this.axios.post(`api/admin/transaction/update`, this.list).then((response) => {
                window.location.reload();
            });
        },changeStatus(id) {
            this.axios.post(`api/admin/transaction/updateIsActive`, {'id': id}).then((response) => {
                window.location.reload();
            });
        },
    },
